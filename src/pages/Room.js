// frontend/src/pages/Room.js (UPDATED with Timer & Run/Submit)
...
import { Play, Send, Loader, Clock, CheckCircle, MessageCircle, Timer } from 'lucide-react';
...
const Room = () => {
  ...
  // Timer states
  const [duration, setDuration] = useState(1200); // 20 minutes default
  ...
  useEffect(() => {
    ...
    socket.current.on('submissionAccepted', (data) => {
      ...
      setShowResults(false); // Hide results modal, show waiting page
    });
  ...
  const handleStartBattle = () => {
    socket.current.emit('startBattle', {
      roomId,
      userId: user.id,
      duration: parseInt(duration) // Send selected duration
    });
  };
  ...
  const getTimerColor = () => {
    ...
    if (remainingTime > 300) return 'text-green-600'; // > 5 mins
    if (remainingTime > 60) return 'text-yellow-600'; // > 1 min
